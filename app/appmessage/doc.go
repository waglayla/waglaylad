/*
Package appmessage implements the waglayla appmessage protocol.

At a high level, this package provides support for marshalling and unmarshalling
supported waglayla messages to and from the appmessage. This package does not deal
...
which waglayla network it is a part of, its type, how big it is, and a checksum
...
of the currently supported waglayla messages are provided. For these supported
messages, all of the details of marshalling and unmarshalling to and from the
appmessage using waglayla encoding are handled so the caller doesn't have to concern
themselves with the specifics.

# Message Interaction

The following provides a quick summary of how the waglayla messages are intended
...
and write waglayla messages. The following sections provide a quick overview of
...
As discussed in the waglayla message overview section, this package reads
and writes waglayla messages using a generic interface named Message. In
...
In order to unmarshall waglayla messages from the appmessage, use the ReadMessage
function. It accepts any io.Reader, but typically this will be a net.Conn to
a remote node running a waglayla peer. Example syntax is:
...
In order to marshall waglayla messages to the appmessage, use the WriteMessage
function. It accepts any io.Writer, but typically this will be a net.Conn to
a remote node running a waglayla peer. Example syntax to request addresses
