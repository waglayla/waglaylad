...
// Outpoint defines a waglayla data type that is used to track previous
// transaction outputs.
...
// NewOutpoint returns a new waglayla transaction outpoint point with the
...
// TxIn defines a waglayla transaction input.
type TxIn struct {
...
// NewTxIn returns a new waglayla transaction input with the provided
...
// TxOut defines a waglayla transaction output.
type TxOut struct {
	Value        uint64
	ScriptPubKey *externalapi.ScriptPublicKey
}

// NewTxOut returns a new waglayla transaction output with the provided
...
// MsgTx implements the Message interface and represents a waglayla tx message.
